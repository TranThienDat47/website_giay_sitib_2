...
<body>
            ...
            <div class="wrap_content">
                ...
                <div class="content_left">
                    <div class="AccountSidebar">
                        ...
                        echo "<p class=\"AccountTitle\">Xin chào <span>{$fullname}</span></p>
                ...
                <div class="content_right" style="width: 75%;">
                    <div class="content__bill" style="overflow-y: auto;height: 100%;">
                        <table class="main-dashboard__order-table" style="width: 100%; font-size: 11px;">
                            <tbody>
                                <tr>
                                    <th style="width: 5%">Mã hoá đơn</th>
                                    <th>
                                        Chi tiết đơn hàng
                                        <hr />
                                        <ul class="main-dashboard__order-detail">
                                            <li style="flex: 1">Sản phẩm</li>
                                            <li style="width: 20%">Màu sắc - Kích thước</li>
                                            <li style="width: 5%">SL</li>
                                            <li style="width: 15%">Giá bán</li>
                                        </ul>
                                    </th>
                                    <th style="width: 12%">Ngày giao dự kiến</th>
                                    <th style="width: 14%">Địa chỉ</th>
                                    <th style="width: 8%">Số ĐT</th>
                                    <th style="width: 8%">Tổng tiền</th>
                                    <th style="width: 10%">Trạng thái</th>
                                    <th style="width: 4%"></th>
                                </tr>
                            </tbody>
                            <tbody class="main-dashboard__order-item-wrapper">
                                ...
                                $sql = "SELECT carts.ID, carts.CART_STATUS, customerpayments.DELIVERY_BEGIN, customerpayments.DELIVERY_END, address.PHONE_NUMBER,
                                        address.DETAIL, address.PROVINCE, address.DISTRICT, address.VILLAGE
                                        FROM carts
                                        JOIN customerpayments ON carts.ID = customerpayments.CART_ID
                                        JOIN address ON customerpayments.ADDRESS_ID = address.ADDRESS_ID
                                        JOIN customers on customers.CUSTOMER_ID = carts.CUSTOMER_ID WHERE customers.CUSTOMER_ID = '" . Session::get("customer_id") . "' ORDER BY carts.ID DESC;";
                                ...
                                function getTotalCartCur($db, $cart_id)
                                {
                                    $sqlTotal = "SELECT SUM(p.price * cd.quantity) AS total_price
                                    FROM products p
                                    ...
                                    $resultTotal = "0";
                                    if ($total) {
                                        $resultTotal = $total['total_price'];
                                    }

                                    return $resultTotal;
                                }

                                function updateStatusCart($db, $cart_id)
                                {
                                    $sql = "UPDATE carts SET CART_STATUS = 'Đã hủy' WHERE ID = $cart_id";
                                    $stmt = $db->query($sql, [$cart_id]);
                                ...
                                if ($_SERVER['REQUEST_METHOD'] == "GET") {
                                    ...
                                    if (isset($_GET['action']) && isset($_GET['cart_id'])) {
                                        ...
                                        if ($action === "cancle_status") {
                                            if (updateStatusCart($db, $cart_id)) {
                                                header("Location: ./information_orders.php", true, 302);
                                                exit();
                                            }
                                        }
                                ...
                                function getAllDetail($db, $cart_id)
                                {

                                    $list_details = "";

                                    $sql = "SELECT products.NAME, products.PRICE, cartdetails.COLOR, cartdetails.SIZE, cartdetails.QUANTITY
                                    FROM products
                                    JOIN cartdetails
                                    ON products.PRODUCT_ID = cartdetails.PRODUCT_ID
                                    WHERE cartdetails.CART_ID = $cart_id;";

                                    $result = $db->fetchAll($sql);

                                    foreach ($result as $row) {

                                        $price = (int) $row["QUANTITY"] * (int) $row["PRICE"];

                                        $price_string = number_format($price, 0, ',', '.');

                                        $list_details .= "
                                        <ul class='main-dashboard__order-detail main-dashboard__order-detail-content'>
                                            <li style='flex: 1'>
                                                <div class='main-dashboard__order-item-proc'>
                                                    <h4 style='font-size: 13px; margin: 0;'
                                                        class='main-dashboard__order-item-title'>
                                                        {$row["NAME"]} ({$row["COLOR"]})
                                                    </h4>
                                                </div>
                                            </li>
                                            <li style='width: 20%'>{$row["COLOR"]} - {$row["SIZE"]}</li>
                                            <li style='width: 5%'>{$row["QUANTITY"]}</li>
                                            <li style='width: 15%'>$price_string ₫</li>
                                        </ul>
                                        ";
                                    }
                                ...
                                foreach ($rows as $temp) {
                                    ...
                                    $price_string = number_format(getTotalCartCur($db, $temp['ID']), 0, ',', '.');

                                    $province = explode("=", $temp['PROVINCE'])[1];
                                    $district = explode("=", $temp['DISTRICT'])[1];
                                    $ward = explode("=", $temp['VILLAGE'])[1];
                                    $begin_deliverly = str_replace("-", "/", explode(" ", $temp['DELIVERY_BEGIN'])[0]);
                                    $end_deliverly = str_replace("-", "/", explode(" ", $temp['DELIVERY_END'])[0]);

                                    $show_huy = "";

                                    $cart_status = $temp['CART_STATUS'];

                                    if ($cart_status === "Chờ xác nhận") {
                                        $show_huy = "<button type='button' class = 'btn_cancle_status'
                                        style='padding: 0px 6px; background-color: #fff; border-radius: 4px; cursor: pointer; border-color: orange; color: orange;'>Hủy</button>";
                                    }

                                    $list_details = getAllDetail($db, $temp['ID']);
                                    ...
                                    $view_order = "
                                                    <tr class='main-dashboard__order-item'>
                                                        ...
                                                        <td class='main-dashboard__order-item-total-price'>
                                                            <span>{$begin_deliverly} đến {$end_deliverly}</span>
                                                        </td>
                                                        <td class='main-dashboard__order-item-total-price'>
                                                            <span>{$temp['DETAIL']}, {$ward}, {$district}, {$province}</span>
                                                        </td>
                                                        <td class='main-dashboard__order-item-total-price'>
                                                            <span>{$temp['PHONE_NUMBER']}</span>
                                                        </td>
                                                        <td class='main-dashboard__order-item-total-price'>
                                                            <span>$price_string ₫</span>
                                                        </td>
                                                        <td>
                                                            <span>{$temp['CART_STATUS']}</span>
                                                        </td>
                                                        <td>
                                                            {$show_huy}
                                                        </td>
                                ...
                                <!-- <tr class="main-dashboard__order-item">
                                    ...
                                    <td class="main-dashboard__order-item-list">
                                        <ul class="main-dashboard__order-detail main-dashboard__order-detail-content">
                                            ...
                                            <li style="width: 20%">Den - 32</li>
                                            <li style="width: 5%">12</li>
                                            <li style="width: 15%">1123</li>
                                        </ul>
                                    </td>
                                    <td class="main-dashboard__order-item-total-price">
                                        <span>11/11/2023 - 14/11/2023</span>
                                    </td>
                                    <td class="main-dashboard__order-item-total-price">
                                        <span>111 số 47 tân quy q7</span>
                                    </td>
                                    <td class="main-dashboard__order-item-total-price">
                                        <span>0222222222</span>
                                    </td>
                                    <td class="main-dashboard__order-item-total-price">
                                        <span>1123 ₫</span>
                                    </td>
    ...
    <script>
        const list_btn_huy = document.querySelectorAll(".btn_cancle_status");

        for (let temp of list_btn_huy) {
            temp.onclick = () => {
                const cart_id = (temp.parentNode.parentNode.children[0].children[0].textContent.trim());
                const confirmed = prompt("Bạn có chắc chắn muốn hủy không? Nhập 'Ok' để xác nhận.");
                if (confirmed === "Ok") {
                    window.location.href = `./information_orders.php?action=cancle_status&cart_id=${cart_id}`;
                }
            };
        }
    </script>
